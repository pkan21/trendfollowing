#!/usr/bin/env python3
"""
Trend Following Signal System — Streamlit + Polygon.io
======================================================
...
Components:
  ...
  2. Vol-Adjusted Slope (25%)     — Clenow exp-regression / vol
...
Score 70–100 → BUY | 40–69 → HOLD | 0–39 → AVOID
"""
...
<style>
    ...
    .signal-buy  { color: #3fb950; font-size: 2.4rem; font-weight: 800;
                   letter-spacing: 0.05em; }
    .signal-hold { color: #d29922; font-size: 2.4rem; font-weight: 800;
                   letter-spacing: 0.05em; }
    .signal-avoid{ color: #f85149; font-size: 2.4rem; font-weight: 800;
                   letter-spacing: 0.05em; }
...
def _get_polygon_client() -> RESTClient:
    """Return a Polygon REST client using the env-var API key."""
    api_key = os.environ.get("POLYGON_API_KEY", "")
    if not api_key:
        raise RuntimeError(
            "POLYGON_API_KEY environment variable is not set.\n"
            "Export it before running:  export POLYGON_API_KEY=your_key_here"
        )
...
def fetch_data(ticker: str, years: int = 2) -> pd.DataFrame:
    """
    Pull daily OHLCV bars from Polygon.io /v2/aggs/ticker/ endpoint.
    Fetches (years + 1) calendar years so look-back windows have enough
    history to warm up (252-day momentum needs ~12 months before first signal).
    """
    ticker = ticker.upper().strip()
    client = _get_polygon_client()

    end_date = dt.date.today()
    start_date = end_date - dt.timedelta(days=int(years * 365 + 365))

    # list_aggs auto-paginates through the full date range
    aggs = list(
        client.list_aggs(
            ticker=ticker,
            multiplier=1,
    ...
    if not aggs:
        raise ValueError(
            f"No data returned for '{ticker}'. Verify the ticker is a valid "
            f"US equity symbol on Polygon.io."
        )
    ...
    if len(df) < 252:
        raise ValueError(
            ...
            f"(need 252+). The ticker may be too new or delisted."
        )
...
# --- 1. Time-Series Momentum (30%) ---

def compute_ts_momentum(close: pd.Series) -> pd.Series:
    """12-month return minus last month, percentile-ranked over trailing year."""
    ...
    score = mom.rolling(252, min_periods=126).apply(
        ...
        if len(x.dropna()) > 10 else 50,
        raw=False,
    )
    return score.clip(0, 100)


# --- 2. Vol-Adjusted Slope (25%) ---
...
def compute_vol_adjusted_momentum(close: pd.Series) -> pd.Series:
    """Clenow 125-day exp-regression slope / annualised vol, percentile-ranked."""
...
# --- 3. Moving Average Regime (20%) ---

def compute_ma_regime(close: pd.Series) -> tuple:
    """Dual-MA system: price vs 10m SMA + 50d EMA vs 200d EMA."""
...
def compute_adx_score(high: pd.Series, low: pd.Series,
                      close: pd.Series) -> tuple:
    """ADX score (0-100) and discount factor for other components."""
    ...
    discount = discount.fillna(0.5)  # ADX < 20 → 50 % discount
    return score, discount, adx


# --- 5. 52-Week High Proximity (10%) ---
...
def compute_composite(df: pd.DataFrame) -> pd.DataFrame:
    """Run all 5 components and produce the composite trend score."""
    ...
    r = df.copy()
    r["ts_momentum"] = ts_mom
    r["vol_adj_momentum"] = vol_mom
    r["ma_regime"] = ma_score
    r["adx_score"] = adx_score
    r["adx_raw"] = adx_raw
    r["adx_discount"] = adx_discount
    r["hw_proximity"] = hw_score
    r["composite_raw"] = composite
    r["composite"] = composite_norm
    r["sma_10m"] = sma_10m
    r["ema_50"] = ema_50
    r["ema_200"] = ema_200
    r["high_52w"] = high_52w
...
def interpret_signal(row: pd.Series) -> dict:
    ...
    components = [
        {"name": "Time-Series Momentum", "weight": "30%",
         "score": row["ts_momentum"], "interp": i_mom(row["ts_momentum"])},
        {"name": "Vol-Adjusted Slope",   "weight": "25%",
         "score": row["vol_adj_momentum"], "interp": i_vol(row["vol_adj_momentum"])},
        {"name": "MA Regime",            "weight": "20%",
         "score": row["ma_regime"], "interp": i_ma(row["ma_regime"])},
        {"name": "ADX Trend Strength",   "weight": "15%",
         "score": row["adx_score"], "interp": i_adx(row["adx_score"], row["adx_raw"])},
        {"name": "52-Week High Prox.",   "weight": "10%",
         "score": row["hw_proximity"],
         "interp": i_52w(row["hw_proximity"], row["close"], row["high_52w"])},
    ]

    return dict(
        signal=signal, css=css, score=score, confidence=confidence,
...
_RANGE_MAP = {"3M": 63, "6M": 126, "1Y": 252, "18M": 380, "ALL": None}


def build_price_chart(ticker: str, df: pd.DataFrame, window_days: int | None) -> go.Figure:
    """Panel 1: Price line + 50 EMA, 200 EMA, 10m SMA with MA-regime shading."""
    if window_days is not None:
        chart_days = min(window_days, len(df) - 1)
    else:
        chart_days = len(df) - 1
    ...
    # MA-regime background shading
    regime_cat = cdf["ma_regime"].apply(
        lambda v: "bull" if v >= 75 else ("mixed" if v >= 25 else "bear")
    )
    color_map = {
        "bull": "rgba(13,74,46,0.35)",
        "mixed": "rgba(61,56,0,0.25)",
        "bear": "rgba(74,13,13,0.35)",
    }
    prev, block_start = None, dates[0]
    blocks = []
    ...
    for s, e, rt in blocks:
        fig.add_vrect(x0=s, x1=e, fillcolor=color_map[rt], line_width=0)

    fig.add_trace(go.Scatter(
        x=dates, y=cdf["close"], name="Price",
        line=dict(color="#58a6ff", width=2.2),
        hovertemplate="$%{y:,.2f}<extra>Price</extra>",
    ))
    fig.add_trace(go.Scatter(
        x=dates, y=cdf["ema_50"], name="50d EMA",
        line=dict(color="#f0883e", width=1.2, dash="dash"), opacity=0.85,
        hovertemplate="$%{y:,.2f}<extra>50d EMA</extra>",
    ))
    fig.add_trace(go.Scatter(
        x=dates, y=cdf["ema_200"], name="200d EMA",
        line=dict(color="#f778ba", width=1.2, dash="dash"), opacity=0.85,
        hovertemplate="$%{y:,.2f}<extra>200d EMA</extra>",
    ))
    fig.add_trace(go.Scatter(
        x=dates, y=cdf["sma_10m"], name="10m SMA",
        line=dict(color="#7ee787", width=1.2, dash="dashdot"), opacity=0.85,
        hovertemplate="$%{y:,.2f}<extra>10m SMA</extra>",
    ))

    fig.update_layout(
        template="plotly_dark",
        paper_bgcolor="#0d1117", plot_bgcolor="#0d1117",
        title=dict(text=f"{ticker} \u2014 Price & Moving Averages",
                   font=dict(size=16, color="#f0f6fc")),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0,
                    font=dict(size=11, color="#c9d1d9"), bgcolor="rgba(0,0,0,0)"),
        height=420, margin=dict(l=50, r=20, t=55, b=30),
        hovermode="x unified",
...
def build_score_chart(ticker: str, df: pd.DataFrame, window_days: int | None) -> go.Figure:
    """Panel 2: Rolling composite score with BUY/AVOID threshold lines."""
    if window_days is not None:
        chart_days = min(window_days, len(df) - 1)
    else:
        chart_days = len(df) - 1
    ...
    if len(score_s) > 0:
        colors = [
            "#3fb950" if v >= 70 else ("#d29922" if v >= 40 else "#f85149")
            for v in score_s
        ]
        fig.add_trace(go.Bar(
            x=score_s.index, y=score_s.values, name="Score",
            marker_color=colors, showlegend=False,
            hovertemplate="%{y:.1f}<extra>Score</extra>",
        ))
        fig.add_hline(y=70, line_dash="dash", line_color="#3fb950", line_width=1,
                      annotation_text="BUY 70",
                      annotation_font_color="#3fb950")
        fig.add_hline(y=40, line_dash="dash", line_color="#f85149", line_width=1,
                      annotation_text="AVOID 40",
                      annotation_font_color="#f85149")

    fig.update_layout(
        template="plotly_dark",
        paper_bgcolor="#0d1117", plot_bgcolor="#0d1117",
        title=dict(text=f"{ticker} \u2014 Composite Trend Score",
                   font=dict(size=16, color="#f0f6fc")),
        height=280, margin=dict(l=50, r=20, t=50, b=30),
        hovermode="x unified",
        yaxis=dict(gridcolor="#21262d", range=[-5, 105], title="Score"),
        xaxis=dict(gridcolor="#21262d"),
    )
    return fig
...
def level_row(label: str, value: float, price: float,
              is_price: bool = True) -> str:
    if is_price:
...
def main():
    # ── Sidebar ──
    with st.sidebar:
        st.markdown("### \U0001f4c8 Trend Signal System")
        st.markdown(
            '<p style="color:#8b949e;font-size:0.82rem;">'
            "5-component composite trend signal for US equities<br>"
            "Powered by <b>Polygon.io</b></p>",
            unsafe_allow_html=True,
        )
        st.markdown("---")
        ticker = st.text_input(
            "Ticker", value="NVDA", max_chars=10,
        ...
        \U0001f7e2 BUY \u2014 Score 70\u2013100<br>
        \U0001f7e1 HOLD \u2014 Score 40\u201369<br>
        \U0001f534 AVOID \u2014 Score 0\u201339<br><br>
        <b>Components</b><br>
        ...
        Data: Polygon.io \u00b7 2 yr+ daily bars
        </div>
        """, unsafe_allow_html=True)
    ...
    if not analyze_btn and "result" not in st.session_state:
        ...
        <div style="text-align:center;margin-top:100px;">
            ...
            <div style="color:#c9d1d9;font-size:1.3rem;font-weight:700;
                        margin-top:12px;">
                Trend Following Signal System</div>
            <div style="color:#8b949e;font-size:0.95rem;margin-top:8px;">
                Enter a US equity ticker and click <b>Analyze</b></div>
        </div>
        """, unsafe_allow_html=True)
        return
    ...
    if analyze_btn:
        with st.spinner(f"Fetching Polygon.io data & computing signals for {ticker}\u2026"):
            try:
                raw_df = fetch_data(ticker)
                result_df = compute_composite(raw_df)
    ...
    # ── Header: signal / score / confidence ──
    ...
    with h3:
        conf_color = {"HIGH": "#3fb950", "MEDIUM": "#d29922", "LOW": "#8b949e"}[
            result["confidence"]
        ]
        st.markdown(
            f'<div class="metric-card">'
    ...
    # ── Time-range toggle ──
    range_labels = list(_RANGE_MAP.keys())
    if "chart_range" not in st.session_state:
        st.session_state["chart_range"] = "18M"

    range_cols = st.columns(len(range_labels))
    ...
    window_days = _RANGE_MAP[st.session_state["chart_range"]]

    # ── Charts ──
    st.plotly_chart(
        build_price_chart(tkr, result_df, window_days), use_container_width=True
    )
    st.plotly_chart(
        build_score_chart(tkr, result_df, window_days), use_container_width=True
    )

    # ── Two-column: breakdown + key levels ──
    ...
    with col_left:
        ...
        for comp in result["components"]:
            ...
            st.markdown(
                ...
                f'<span class="comp-weight">{comp["weight"]} \u00b7 '
                f'{s:.1f}/100</span></div>'
                f'{bar_html(s)}'
    ...
    with col_right:
        ...
        st.markdown(
            ...
            f'<div class="metric-value" style="font-size:1.8rem;">'
            f'${price:,.2f}</div></div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            f'<div class="metric-card">{levels_html}</div>', unsafe_allow_html=True
        )
        ...
        st.markdown(
            ...
            f'<div class="metric-value">{dist:+.1f}% from 52w high</div></div>',
            unsafe_allow_html=True,
        )
