#!/usr/bin/env python3
"""
Trend Following Signal System v4 — Streamlit + Polygon.io
==========================================================
...
NEW    Backtest tab with equity curve
"""
...
def run_backtest(df: pd.DataFrame) -> dict:
    ...
    # Stats
    strat_ret = (equity.iloc[-1] / 100 - 1) * 100
    bnh_ret = (bnh_equity.iloc[-1] / 100 - 1) * 100
    ...
    strat_dd = max_dd(equity)
    bnh_dd = max_dd(bnh_equity)

    n_trades = len(trades)
    wins = sum(1 for t in trades if t["return_pct"] > 0)
    win_rate = (wins / n_trades * 100) if n_trades > 0 else 0
    avg_dur = np.mean([t["days"] for t in trades]) if trades else 0
...
def main():
    ...
    if not go_btn and "res" not in st.session_state:
        st.markdown(
            ...
            'Trend Following Signal System v4</div>'
            '<div style="color:#6e7a8a;font-size:.92rem;margin-top:8px;">'
            'EMA-smoothed scoring \u00b7 Hysteresis state machine \u00b7 Vol-regime thresholds \u00b7 Backtest<br>'
            'Polygon.io data \u00b7 Enter a ticker above</div></div>',
            unsafe_allow_html=True)
        return
    ...
    with tab_bt:
        if bt is None:
            ...
            st.markdown(f'<div class="card-sub" style="margin-bottom:16px;">'
                        f'Simulated over {len(rdf)} trading days. Enter at next open '
                        f'on BUY, exit at next open when signal drops. No transaction costs.</div>',
                        unsafe_allow_html=True)

            # Stats grid
            c1, c2, c3, c4 = st.columns(4)
            strat_c = "#3fb950" if bt["strat_return"] > 0 else "#f85149"
            bnh_c = "#3fb950" if bt["bnh_return"] > 0 else "#f85149"
            with c1:
                st.markdown(f'<div class="bt-stat"><div class="bt-label">Strategy Return</div>'
                            f'<div class="bt-val" style="color:{strat_c}">'
                            f'{bt["strat_return"]:+.1f}%</div></div>', unsafe_allow_html=True)
            with c2:
                st.markdown(f'<div class="bt-stat"><div class="bt-label">Buy & Hold Return</div>'
                            f'<div class="bt-val" style="color:{bnh_c}">'
                            f'{bt["bnh_return"]:+.1f}%</div></div>', unsafe_allow_html=True)
            with c3:
                st.markdown(f'<div class="bt-stat"><div class="bt-label">Strategy Max DD</div>'
                            f'<div class="bt-val" style="color:#f85149">'
                            f'{bt["strat_dd"]:.1f}%</div></div>', unsafe_allow_html=True)
            with c4:
                st.markdown(f'<div class="bt-stat"><div class="bt-label">B&H Max DD</div>'
                            f'<div class="bt-val" style="color:#f85149">'
                            f'{bt["bnh_dd"]:.1f}%</div></div>', unsafe_allow_html=True)

            c5, c6, c7, _ = st.columns(4)
            with c5:
                st.markdown(f'<div class="bt-stat"><div class="bt-label">Trades</div>'
                            f'<div class="bt-val">{bt["n_trades"]}</div></div>', unsafe_allow_html=True)
            with c6:
                wr_c = "#3fb950" if bt["win_rate"] >= 50 else "#d29922"
                st.markdown(f'<div class="bt-stat"><div class="bt-label">Win Rate</div>'
                            f'<div class="bt-val" style="color:{wr_c}">'
                            f'{bt["win_rate"]:.0f}%</div></div>', unsafe_allow_html=True)
            with c7:
                st.markdown(f'<div class="bt-stat"><div class="bt-label">Avg Duration</div>'
                            f'<div class="bt-val">{bt["avg_duration"]:.0f} days</div></div>',
                            unsafe_allow_html=True)
